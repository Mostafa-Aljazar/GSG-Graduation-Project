'use server';

import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/utils/prisma';
import { verifyJWT } from '@/utils/auth';
import { USER_RANK, COMPLAINT_STATUS } from '@prisma/client'
import { IActionResponse } from '@/types/common/action-response.type';

export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        const token = req.headers.get('authorization')?.replace('Bearer ', '');
        if (!token) return NextResponse.json<IActionResponse>({ status: 401, message: 'غير مصرح' }, { status: 401 });

        const viewer = verifyJWT(token);
        const { id: complaintId } = await params;

        const complaint = await prisma.complaint.findUnique({
            where: { id: complaintId },
            include: { receiver: true, sender: true },
        });

        if (!complaint) {
            return NextResponse.json<IActionResponse>({ status: 404, message: 'الشكوى غير موجودة' }, { status: 404 });
        }

        const canDelete =
            complaint.senderId === viewer.id ||
            (viewer.rank === USER_RANK.SECURITY_OFFICER && complaint.receiver.role === USER_RANK.SECURITY_PERSON);

        if (!canDelete) {
            return NextResponse.json<IActionResponse>({ status: 403, message: 'لا يمكنك حذف هذه الشكوى' }, { status: 403 });
        }

        await prisma.complaint.delete({ where: { id: complaintId } });

        return NextResponse.json<IActionResponse>({
            status: 200,
            message: 'تم حذف الشكوى بنجاح',
        });

    } catch (err: any) {
        return NextResponse.json<IActionResponse>({
            status: 500,
            message: err?.message || 'حدث خطأ أثناء حذف الشكوى',
            error: err?.message || 'خطأ',
        }, { status: 500 });
    }
}
