...
export default function CommonSendComplaint() {
  ...
  const handleSubmit = (values: TCommonSendComplaintFormValues) => {
    ...
    sendCommonComplaintMutation.mutate({
      ...
      role: (user.role == USER_TYPE.SECURITY_PERSON
        ? user.rank
        : user.role) as ISendCommonComplaintProps['role'],
      reception: values.reception as ISendCommonComplaintProps['reception'],
      title: values.title,
